...
int delete(){
    ...
    for (int i = 0; i < env_size; i++) {
        ...
        if (shares != NULL && cJSON_IsArray(shares)) {
            ...
            printf(WHT "\n[>] Enter service ID to delete | Empty to cancel: "); 
            ...
            if (strlen(input) != 0) {
                ...
                fgets(state, sizeof(state), stdin); state[strcspn(state, "\n")] = 0; fflush(stdin);
                toLowerCase(state);

                char command[512];
                if (strcmp(state, "y") == 0 || strcmp(state, "") == 0) {
                    snprintf(command, sizeof(command), "%s release %s", get_runtime_file(), token->valuestring);
                    printf(BMAG "Contacting zrok server...\n"); printf(WHT "");
                }else{
